...
export const useGameGenerator = () => {
  ...
  /**
   * Current matching options (reactive)
   */
  const matchingOptions = ref<MatchingOptions>({ ...defaultOptions });

  /**
   * Generate a new game schedule
   */
  ...
  /**
   * Regenerate the current schedule with same options
   */
  ...
  /**
   * Validate current options against available players
   */
  ...
  /**
   * Calculate average skill difference across all games
   */
  ...
  /**
   * Get resting players for a specific round
   */
  ...
  return {
    ...
    getScheduleStats: readonly(getScheduleStats)
  };
};
